...
def main():
    p = argparse.ArgumentParser(description="Convert AnyLabeling JSONs → DeepLabCut CSV. Use the same directory structure for the input as you expect the structure of your DLC directory \"labeled-data\" to to be. E.g. if DLC-project/labeled-data contains cam-1, cam-2, and cam-3 (directories for separate videos!), put the cam-1.json files in a cam-1 directory and so on.")
