...
class TimedRender(Operator):
    ...
    bl_description = """
        INFO: Cancel periodic rendering by pressing ESC.
...
class SYNTH_OT_apply_settings(Operator):
    ...
    bl_label = "Apply Settings And Setup Directory Structure"
...
class SYNTH_OT_export_keypoint_list(Operator):
    ...
    def execute(self, context):
        ...
        try:
            out_file = os.path.join(resolve(p.annot_out_dir), 'keypoint_list.txt')
    ...
    def execute(self, context):
        ...
        for cam in cam_objs:
            ...
            try:
                ...
                self.report({'WARNING'}, f"Failed to compute matrix for {cam.name}: {e}")
                continue
            def mat_to_list(m):
        ...
        try:
            ...
            self.report({'INFO'}, f"Wrote camera matrices to {out_path}")
            return {'FINISHED'}
        except Exception as e:
            self.report({'ERROR'}, f"Failed to write cam_matrices.json: {e}")
            return {'CANCELLED'}
